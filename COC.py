...
# Function to calculate mass from concentration
...
# Function for dilution calculations
...
if menu == "Home":
    ...
    st.write("""
        ...
        Silakan pilih menu di sidebar untuk mulai menggunakan.
    """)
...
# Conversion Page with separate Atomic Mass calculation
elif menu == "Konversi":
    st.header("Konversi Satuan")
    
    tab1, tab2 = st.tabs(["Konversi Satuan", "Massa Atom Relatif"])
    ...
    with tab1:
        ...
        to_unit = st.selectbox("Ke satuan:", ["Molaritas (g/mol)", "Normalitas (g/grek)", "% (b/v)", "PPM (mg/L)"])
        
        if st.button("Konversi"):
            if from_unit == to_unit:
                st.warning("Anda memilih satuan yang sama!")
            else:
                # Simple conversion (would need proper conversion factors for real implementation)
...
# About Page
    ...
    - Andi Muhammad Tegar A A
    - Inezza Azmi Tobri
    - Muhammad Habibie Rasyha
    - Saskia Putri Irfani
    - Zahra Nandya Putri N
